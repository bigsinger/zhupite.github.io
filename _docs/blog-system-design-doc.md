...
| 约束 | 原因 |
|------|------|
| 不动 `_config.yml` | Jekyll 构建入口，任何改动都可能破坏构建 |
...
| 不能破坏 `//` 行注释 | compress_html 插件会吞噬换行，详见 9.1 |
...
| TOC 活跃章节 | `IntersectionObserver` | 基于 `rootMargin: '-20px 0px -60% 0px'` 的可见性判断 |
...
| 19 | Responsive | L1505-1517 | 响应式断点 |
| — | Old compat | L1519+ | 旧样式兼容 |
...
| `_config.yml` | ❌ | — | ❌ | 构建入口 |
| `_docs/` | ✅ | ✅ | ✅ | 项目文档，自由操作 |
| `assets/js/` | ❌ | — | ❌ | JS 文件不动（改 inline 脚本除外） |
| `assets/vendor/` | ❌ | — | ❌ | 第三方库 |
...
| 活跃跟踪 | `IntersectionObserver` + `rootMargin: '-20px 0px -60% 0px'` |
...
| P1 | 重写 TOC 内联脚本为外部 JS | 当前 TOC JS 在 `sidebar-post.html` 中 inline，可提取到 `assets/js/` | 中 |
...
| 文件 | 用途 | 说明 |
|------|------|------|
| `assets/css/theme-modern.css` | 核心设计系统 | 1520 行，20+ 模块，持续增长 |
| `assets/css/bundle-legacy.css` | 6 个旧 CSS 合并 | 15KB，兼容层，不改动 |
| `_includes/sidebar-post.html` | 侧边栏 + TOC + 搜索 + 分类 | TOC JS 内联在此 |
| `_includes/header.html` | 头部导航 | sticky + 暗色切换 + hamburger |
| `_includes/footer.html` | 底部 | 极简 + 统计 |
| `index.html` | 首页模板 | 卡片网格 + 侧边栏 + 分页器 |
| `_layouts/post.html` | 文章详情页 layout | 正文 + TOC + 评论 |
