...
| 🔍 **全文搜索** | SimpleJekyllSearch 客户端实时模糊匹配，无后端依赖；侧栏 + 移动端覆盖层双搜索框共享同一数据源，移动端额外匹配分类和关键字 |
| ⏪ **上一篇/下一篇** | 文章底部卡片式导航，首末篇优雅禁用 |
| 📊 **字数统计** | 精确统计 + 阅读时长（350字/分钟），文章顶部显示 |
|| 📌 **文章置顶** | 首页优先显示，右上角「★ 置顶」毛玻璃徽章，无装饰条 |
