...
强烈推荐使用 [Magisk](https://github.com/topjohnwu/Magisk/releases) 为设备获取 Root 权限，参考如下步骤可以分分钟完成：

1. 手机上安装应用：[Magisk](https://github.com/topjohnwu/Magisk/releases) 。
2. **将 `boot.img` 文件传输到设备**： 将手机系统对应的的 `boot.img` 文件传输到设备的内部存储或 SD 卡中。如果是ROM的系统，则可以在ROM包里面找到。

4. **使用 Magisk 修补 `boot.img` 文件**：  
...
6. **进入 Fastboot 模式**：`adb reboot bootloader`，或者关闭设备，使用音量减键和电源键组合进入 Fastboot 模式。
7. **刷入修补后的 `boot.img` 文件**：  使用 USB 线将设备连接到电脑，输入以下命令刷入修补后的文件：
...
- `Huawei USB COM 1.0 Driver`驱动的安装不必提前进行，在短接的时候电脑会自动识别设备，并在设备管理器中自动会出现，如果未能正确显示出类`USB COM 1.0`这样的端口，这个时候再安装驱动即可。技巧：这里不用网上满世界找这个驱动，只需要安装[华为手机助手HiSuite](https://consumer.huawei.com/cn/support/hisuite/)即可，驱动会自动安装。
...
### 2026-6-30
相关资料：
- 短接点：[SNE-AL00 Test point](https://edl.unlocktool.net/mate-20-lite-sne-al00-test-point)，图片参考：![](https://edl.unlocktool.net/uploads/huawei/Mate%2020%20Lite%20(SNE-AL00)%20Test%20point.png)
- 短接方法：关机 → 露出 TestPoint → 先短接 TP 到 GND/屏蔽罩 → 再插 USB 供电/连电脑 → 电脑出现 USB_SER/HUAWEI USB COM 1.0 后松开。不要在不确定 TP 点位时带电乱戳；镊子只碰 TP 和 GND/屏蔽罩，别碰旁边小电容/电阻。
- 参考教程：[指南：如何在华为Mate 20 Lite上解锁FRP（谷歌账号）- Techolay Social](https://techolay.net/sosyal/konu/huawei-mate-20-lite-frp-google-hesabi-kilidi-nasil-acilir.175641/?__cf_chl_f_tk=28_gayCcscv5R9ZVgm1NxxfjzuF52hC3Car1NtfCYNM-1782784025-1.0.1.1-Ghvf07jdAYnd0SGngCTVVbECg.WFt.mZ9_dbOppB4_k)
- ADB 驱动：https://xiaomitools.com/download/adb-installer-v1-4-9/
- 华为 VCOM 驱动：通过安装[华为手机助手HiSuite](https://consumer.huawei.com/cn/support/hisuite/)自动安装。
- 麒麟工具（Kirin-Tool）：https://kirintool.cfd ，截至目前版本为： v2.4.2，解锁很方便，只要短接成功就可以成功解锁。

解锁步骤：
- 不确定是否需要降级系统到`EMUI 9.1`，之前因为尝试解锁过，所以系统曾经降级过，目前已经是`EMUI 9.1`。
- 关闭手机，打开后盖，取出电池排线。将USB数据线连接到设备，用镊子夹住手机上黄色排线旁边的黄色圆点和排线顶部的金属部分，保持短接，同时把 USB-C 插入手机，等待 2～5 秒，电脑设备管理器出现 `USB_SER` 或 `HUAWEI USB COM 1.0`
松开镊子。注意：如果手机屏幕亮起充电动画、华为 Logo、eRecovery、升级界面，通常说明没有进 VCOM/TestPoint；应断线、关机或断电池，重新“短接后再插 USB”。
- 打开设备管理器，你会看到一个名为 USB_SER 的设备。将华为 VCOM 驱动程序解压到一个文件夹，然后返回设备管理器，右键单击 USB_SER 设备，选择“更新驱动程序”。选择“浏览计算机以查找驱动程序软件”后，单击“浏览”找到并选择你的驱动程序文件夹。如果你的操作系统是 64 位，请选择 x64 文件夹；如果是 32 位，请选择 x86 文件夹。单击“确定”，然后单击“下一步”。如果出现警告，请单击“仍然安装”。
- 将麒麟工具（Kirin-Tool）解压到一个文件夹，运行主程序。如果提示需要安装.NET框架，就安装即可。
- 点击“选择 CPU 型号”，向下滚动，选择麒麟 710（`CPU Model: Kirin 710 / hisi710`
不要选 Kirin 710A，不要选 Kirin 710F）。然后，转到 VCOM 操作选项卡，点击“解锁引导程序”旁边的“解锁”按钮，并等待。如果程序卡住，请关闭程序并执行手机短接操作，重复短接操作并重新打开程序。
- 进入 VCOM 操作，在顶部的 CPU 型号中选择 `Kirin 710`，然后点击“解锁”；解锁过程将完成（我就是这么做的）。返回设备管理器，你会看到一个名为“Android”的设备。右键单击该设备，选择“更新驱动程序”，然后点击“让我从计算机上的可用驱动程序列表中选择”。选择“Android 设备”，点击“下一步”，再次点击“下一步”，然后点击“是”安装驱动程序。
- 关闭并重新打开 Kirin Tool（以防万一），在 CPU 型号中选择 `Kirin 710`，进入“安全工具”选项卡，点击`Unlock Bootloader`。可以在同一次连接、同一次 `factory/unlocked fastboot` 会话里点`FRP Lock Removal`。
- 过程完成后，连接手机电池和排线，然后开机。手机将格式化并启动，FRP 锁将被移除。
- 完成后再用 `platform-tools` 检查：
```
fastboot devices
...
unlocked: yes
```
如果只显示临时 `unlocked`，重启后又变回 `locked`，就说明你只完成了 `VCOM/Factory Fastboot` 阶段，没有完成永久 `Bootloader unlock`。

验证完成解锁。


#### ROM
- 教程来源：[[ROM][13.0][ LineageOS 20 - Huawei Mate 20 Lite - sydneym ][ Update 24/06/2026 ] - XDA Forums](https://xdaforums.com/t/rom-13-0-lineageos-20-huawei-mate-20-lite-sydneym-update-24-06-2026.4714244/) 需要注意的是：这个帖子一开始传的文件有误(漏掉了更新`recovery.img`)，不能按照它提供的文件操作，正确的操作方法我贴在后面了。
- 下载地址：[lineage-20.0-20260624-RELEASE-sydneym.zip](https://mega.nz/file/pJVyUB7Q#bNBBr8f0BsbYwvz50wkJDAmCGv_FIeX3jFnghu1x3N8)


操作步骤：
- 保证手机已经解锁，然后进入`fastboot`模式。
- 安装`Lineage OS Recovery`。从`lineage-20.0-20260624-RELEASE-sydneym.zip`压缩包里提取如下的img包并刷入手机。
```
fastboot flash kernel boot.img
fastboot flash recovery_ramdisk recovery.img
fastboot flash recovery_vendor recovery_vendor.img
```
- 手动进入LOS的恢复模式。注意这里使用命令`fastboot reboot recovery`是无法进入恢复模式的，只会进入fastboot模式。需要手动方式进入，参考方法：[华为手机系统修复模式下，“恢复出厂设置”按钮无法点击 - 华为官网](https://consumer.huawei.com/cn/support/content/zh-cn15822537/)，我实践误打误撞的方法：关机、不插电源线、音量上+电源、然后按音量下。
- 在LOS的恢复模式下安装`Lineage OS ROM `：
```
Factory reset -> Format data/factory reset
Apply update -> Apply from ADB
adb sideload lineage-20.0-20260624-RELEASE-sydneym.zip
```
不过这一步骤有错误：
```
...
```
GApp不用安装，这个是谷歌服务。如果需要安装则：adb sideload [MindTheGapps-13.0.0-arm64-20231025_200931.zip](https://github.com/MindTheGapps/13.0.0-arm64/releases/tag/MindTheGapps-13.0.0-arm64-20231025_200931)
- 重启系统。
...
#### ROOT
Install [KernelSU-Next](https://github.com/KernelSU-Next/KernelSU-Next) app to ROOT your device !

Installation of Magisk :

~ install the last update of [Magisk App](https://github.com/topjohnwu/magisk/releases) ~

extract boot.img from LOS.zip ( or use the one above )

into Magisk you must to patch this boot.img

extract from device the patched boot.img
...
2. 找到主板上的 Test Point（短接点），不同机型位置不同，可以参考：
   ...
   - 华为麦芒 7（SNE-AL00）短接点（需要查找该机型的 [Test Point 图片](https://haitunrom.com/djt/huawei/5823.html)）
...
## Redmi3S
小米 Redmi 3s 于 2016 年 6 月发布，是一款价格亲民的入门级手机，其硬件配置与其价格相符。该机搭载高通骁龙 430 处理器，配备 1.4GHz 八核 CPU、Adreno 505 GPU 和 2GB 运行内存。16GB 的机身存储空间支持最高 256GB 的 microSD 卡扩展。机身重量仅为 144g，内部配备一块 5 英寸 720P IPS LCD 显示屏，像素密度约为 294 PPI。整机由一块不可拆卸的 4100mAh 电池供电。

- 小米手机红米3S专用的TWRP for Redmi 3s(land)可以从这里下载：[Download TWRP for land](https://dl.twrp.me/land/) ,刷LOS系统时无须使用，这里列出来仅作为以后查询。
- LOS官方支持Redmi 3S的包下载地址：[Redmi 3S / 3X / 4(X) / [Note 5A / Y1 - Prime] LineageOS Downloads](https://download.lineageos.org/devices/Mi8937/builds) 
- LOS刷机官方教程：[Install LineageOS on Mi8937 - LineageOS Wiki](https://wiki.lineageos.org/devices/Mi8937/install/variant1/)
- LOS刷机也可参考：[【指南/教程/操作指南】小米 Redmi 3S 升级到 Android 15 - XDA 论坛](https://xdaforums.com/t/guide-tutorial-howto-xiaomi-redmi-3s-stock-to-android-15.4742885/)
- 后期可以关注板块：[小米 Redmi 3s 指南、新闻和讨论 - XDA 论坛](https://xdaforums.com/f/xiaomi-redmi-3s-guides-news-discussion.5978/)


**2025-12-16 LOS23 Android16**：
参考XDA论坛的帖子：[Official LineageOS 23 is out! - XDA Forums](https://xdaforums.com/t/official-lineageos-23-is-out.4557229/#post-90326959)得知应该是可以刷的。

有Git记录为证：[hudson: Ship Mi8917 & Mi8937 & Mi439 with lineage-19.1 · LineageOS/hudson@8355202](https://github.com/LineageOS/hudson/commit/835520206014f6b42a003bc11594bd00c64441b7)

以及官方也有推荐：[Redmi 3S / 3X / 4(X) / [Note 5A / Y1 - Prime] LineageOS Downloads](https://download.lineageos.org/devices/Mi8937/builds) 

之前没能刷成功是因为不能使用TWRP/PBRP，需按照这个指南操作：
需要参考：[Install LineageOS on Mi8937 - LineageOS Wiki](https://wiki.lineageos.org/devices/Mi8937/install/variant1/)


主要操作步骤：
1. 准备刷机工具，主要是`adb`和`fastboot`。可以从这里下载小米官方提供的刷机工具包。
2. 解锁并安装驱动。（因为我这个手机之前刷机解锁过，此次就跳过了）
3. 刷入LOS的`recovery.img`，然后进入恢复模式。
4. 在恢复模式下，点击“恢复出厂设置”，然后点击“格式化数据/恢复出厂设置”，并继续完成格式化过程。然后通过侧载的方式安装LOS的zip包，安装完成后重启即可。

整体非常方便，这次几分钟就完成了。重启速度非常快，首次启动也比之前官方的ROM刷入后首次启动快的多。
LOS23刷的是Android16的系统，没想到能在这么老的手机设备上安装安卓16系统，也是没想到的，这感觉就好比使用iPhone6给刷上了iPhone17的系统。

**2025-12-16 官方ROM刷机**
之前刷机的时候使用的TWRP，可能清除了不该清除的分区导致的蓝牙和WiFi异常不可用。GPT给的建议也是：硬件损坏的概率依然很低（Redmi 3S 的 Wi-Fi/BT 芯片非常耐操）。

下载官方ROM包（[红米 3S/X(land) 官方ROM下载](https://roms.miuier.com/zh-cn/devices/land/)），国际稳定版就行，例如： `land_global_images_V10.2.2.0.MALMIXM_20190423.0000.00_6.0_global`，解压后执行：`flash_all.bat`即可，基本上是一键刷机。

分分钟就刷好了，官方ROM刷入后第一次重启很慢，要耐心等待，我当时可能就是因为这个时间过长误以为没有刷机成功然后就做了其他操作导致的。

这个重启的时间远超过了刷机的时间，耐心等待，重启后检查蓝牙和WiFi可用。

**模拟联网**
之前刷机的时候使用的TWRP，可能清除了不该清除的分区导致的蓝牙和WiFi异常不可用。

试过很多方法没能恢复，实在不想折腾了，于是想到能不能通过USB连接电脑的形式来给手机模拟网络。咨询了KIMI得到了一个很简单方便的方法：
1. 执行如下命令：
```
adb shell svc usb setFunctions rndis
```
电脑的网络链接立马出现一个`Remote NDIS based Internet Sharing Device #2`类似的网卡（或者类似`Android USB Ethernet/RNDIS`）.
2. 电脑打开：控制面板\网络和 Internet\网络连接，右键当前上网的网卡（如以太网或WiFi）→ 属性 → 共享 → 勾选“允许其他用户通过此计算机的Internet连接”。在下拉框中选择上述新出现的RNDIS网卡 → 确定。
3. 手机端无需额外设置，系统会自动通过USB获取IP并上网。
...
这部手机太老了，本来计划去年国庆的时候卖掉的，结果忘记了，就一直保留到现在。充了一次电，待机居然一个多礼拜，真够强悍的。因为要做一个小测试，发现模拟器不行，必须使用真机才行，手头没有真机root设备。网上了解了下，很多技术在Android系统6.0以上还是可以适用的，而这部`Redmi3S`的系统是6.0.1的，用来测试还是可以的，于是打算root掉。
...
5. 进入 Fastboot 模式：`adb reboot bootloader`，或者关机，按住音量减 + 电源键，出现 fastboot 模式界面时松开所有键。将手机连接到 PC。
