...
**运维工作流：**
首次部署：`npm create astro@latest` 创建项目 → 编写内容 → `npm run build` 构建静态文件 → 将 `dist/` 目录上传到 GitHub Pages / Netlify / Vercel。若使用 SSR 模式需额外配置 Adapter（Node/Vercel 等）。

新增文章：在 `src/content/blog/` 下新建 `.md` 或 `.mdx` 文件，写入 frontmatter + 正文，重新构建即可。

日常发布：纯静态模式下每次 `npm run build` 后推送 `dist/` 到托管平台；推荐配置 CI/CD（GitHub Actions / Netlify 自动部署），git push 即自动完成构建发布。
...
**运维工作流：**
首次部署：下载 Hugo 二进制（单文件）→ `hugo new site myblog` 创建项目 → 选择主题 → 编写内容 → `hugo` 命令构建，`public/` 目录即为输出。可部署到任何静态托管。

新增文章：`hugo new content posts/my-new-post.md` 自动生成带 frontmatter 的模板文件，或手动在 `content/posts/` 下创建 `.md` 文件。

日常发布：推荐 GitHub Actions + GitHub Pages：配置 workflow 在 `git push` 时自动 `hugo` 构建并部署。因构建极快（亚秒级），本地也可一键 `hugo` 后手动上传。
...
**运维工作流：**
首次部署（GitHub Pages）：fork 或 `jekyll new myblog` 创建项目 → 推送到 `username.github.io` 仓库 → GitHub 自动识别并构建发布，无需任何配置。非 GitHub Pages 场景需在服务器上安装 Ruby + Jekyll 后构建。

新增文章：在 `_posts/` 下创建 `YYYY-MM-DD-title.md` 文件，写入 frontmatter + 正文即可。文件名中的日期即发布日期。

日常发布：最常见的方式是 `git push` 到 GitHub 仓库，Pages 自动构建部署；也可本地 `jekyll build` 后上传 `_site/` 目录。注意 GitHub Pages 模式下禁用了非白名单插件。
...
**运维工作流：**
首次部署：`npm install -g hexo-cli` → `hexo init myblog` → `cd myblog && npm install` → `hexo server` 本地预览 → `hexo generate` 构建静态文件输出到 `public/`。支持一键部署到 GitHub Pages（需配置 `_config.yml` 中的 deploy 字段）。

新增文章：`hexo new "文章标题"` 自动在 `source/_posts/` 下创建带 frontmatter 的 `.md` 文件，或手动新建后写入内容。

日常发布：`hexo generate && hexo deploy` 可一键构建并推送到 GitHub Pages、Netlify、Vercel 等。也可配置 GitHub Actions 实现 `git push` 自动构建发布。
...
**运维工作流：**
首次部署：下载 Grav 压缩包 → 解压到 PHP Web 服务器目录（如 Apache/Nginx 的 wwwroot）→ 确保 PHP 8.3+ 及所需扩展已安装 → 浏览器访问域名进入安装向导。如需管理后台，额外安装 Admin 插件。

新增文章：两种方式——① 通过 Admin 后台可视化编辑器在线撰写发布；② 直接在 `user/pages/` 下创建文件夹和 `item.md` 文件，支持 Markdown + YAML frontmatter。

日常发布：因 Grav 是动态系统，文件上传即生效，无构建步骤。内容变更只需通过 FTP/SFTP 同步文件或直接在后台编辑即可。注意没有 CI/CD 自动构建概念——文件到位即上线。
