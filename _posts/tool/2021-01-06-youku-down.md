...
优酷近年已逐步弃用旧的 .kux 专有加密格式，转向标准 HLS 流媒体分发。因此推荐策略已变：不再依赖 kux→mp4 转换，而是直接使用能解析流地址并下载的工具。以下将2025-2026年推荐的方案放在前面，老方案保留在后供参考。
...
[you-get](https://github.com/soimort/you-get)（Stars 56,800+）支持优酷（youku.com）在线视频下载，稳定可靠。

> 🎯 **推荐理由**：you-get 直接解析优酷在线视频流，绕过整个 kux 加密格式的麻烦环节。
...
# 下载优酷视频
you-get "https://v.youku.com/v_show/id_xxx.html"

# 查看可用画质
you-get -i "URL"

# 指定画质
you-get "URL" --format=hd3
```

## Lux（Go语言高速下载）

[lux](https://github.com/iawia002/lux)（Stars 31,387，Go语言）支持优酷视频在线下载，Go单文件编译，速度极快。

```bash
# 下载优酷视频
lux "https://v.youku.com/v_show/id_xxx.html"
...
如果你手里还保存着旧的 .kux 文件，可以使用以下工具转换：

### jsjtsty/KUXConverter（C++，推荐）

[jsjtsty/KUXConverter](https://github.com/jsjtsty/KUXConverter)（Stars 39）是一个轻量级的 .kux→.mp4 转换工具，C++编写，直接将优酷加密 kux 文件转为标准 MP4。工具本身功能稳定，虽已较久未更新但转换功能仍可用。

```bash
# 使用
KUXConverter.exe input.kux output.mp4
```

## N_m3u8DL-RE（m3u8流下载）

[N_m3u8DL-RE](https://github.com/nilaoda/N_m3u8DL-RE)（Stars 8,100+）是原N_m3u8DL-CLI的跨平台升级版。不过请注意：N_m3u8DL的旧版本曾标注"不支持优酷视频解密"，新版需要实际测试。

## ChattyPlay-Agent（一站式全平台）

[ChattyPlay-Agent](https://github.com/P1kaj1uu/ChattyPlay-Agent)（Stars 678）支持优酷等20+平台在线解析下载。

## 推荐路线（2026年6月）

| 使用场景 | 推荐方案 |
|---------|---------|
| 在线下载优酷视频 | **you-get** 或 **lux** |
| 已有 .kux 文件需转换 | **jsjtsty/KUXConverter** |
