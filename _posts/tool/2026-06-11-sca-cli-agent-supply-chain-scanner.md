...
当你的 AI Agent 技能包里有一个硬编码的 `subprocess.run(["pg_dump", ...])` 调用时，传统 SCA 工具能发现吗？不能——因为 `manifest` 里只声明了 `psycopg2`，但 Agent 运行时确实需要系统中的 `pg_dump` 可执行文件。这不是传统 SCA 工具的疏忽，而是它们的基因里就没有为 AI Agent 场景设计过。

**sca-cli 不是传统 SCA 工具。** 它是专为 AI Agent 时代设计的供应链安全扫描器——不满足于读 `manifest`，而是深入代码层，扫描你在源码中「写死了什么依赖」：import 了什么、调用了什么外部命令、依赖了哪些系统可执行文件。它能发现 `pg_dump`、`psql`、`ffmpeg`、`curl` 这些不在任何 `package.json` 里、但 Agent 运行时确实需要的外部系统工具依赖。一份 16 字节的 zip 包，经过 sca-cli 扫描后可产出 16KB 的完整 CycloneDX SBOM——其中嵌入漏洞信息、许可证数据和风险评级。**一份文件，就是完整的可交付工件。**

sca-cli 完全本地运行、零网络依赖。从 SBOM 生成到漏洞匹配、从规则扫描到报告输出，全部在本地完成，无需 API Key、无需互联网连接。这对政府、金融、关键信息基础设施等离线环境是刚性需求，也是企业级供应链安全治理的前提条件。
