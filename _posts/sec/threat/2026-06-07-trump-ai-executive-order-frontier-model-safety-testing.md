...
| 章节 | 内容 | 时间要求 |
|------|------|---------|
| **Section 2** | 升级美国系统的网络防御，CISA 发布约束性操作指令 | 30 天内 |
| **Section 3** | 建立「前沿模型安全测试」的自愿框架（核心争议点） | 2 个月内制定细则 |
| **Section 4** | 司法部长追查 AI 驱动的网络犯罪 | 即时 |
| **Section 5** | 其他（免责条款等） | — |

最受关注的 Section 3 具体包含：

1. **分类基准测试流程**：由多个机构联合制定一套**保密**的基准，评估 AI 模型的网络攻防能力，以此判定哪些模型属于「受覆盖的前沿模型」
...
行政令的文字刻意避开了「许可」「预审」等字眼，但政策分析人士普遍认为这实质上是**事实上的许可制**（de facto licensing regime）。The Zvi 文章的标题直言："Welcome to The Prior Restraint Era"（欢迎来到预先限制时代）。
...
**理由二：分类基准缺乏透明度**
如果连触发审查的计算阈值都是保密的，实验室研究人员自己都不知道训练中的模型是否落入监管范围。Dean Ball 直言质疑：**把基准内容和审查结果都保密，比单独保密基准本身危险得多。**

**理由三：30 天窗口无硬性截止**
行政令未规定政府在 30 天内必须做出决定的最终时限，也未规定逾期未决的默认通过机制。这意味着审查可能无限期拖延。
...
| 阵营 | 代表人物 | 立场 | 结果 |
|------|---------|------|------|
| **安全派** | 国家安全团队、部分共和党议员 | 需要监管，不能什么都不做 | ✅ 胜利 |
| **加速主义派** | David Sacks、Elon Musk | 零监管，认为任何约束都会扼杀创新 | ❌ 失败 |
| **中间派** | Dean Ball、Neil Chilson | 需要监管但要谨慎，避免误伤 | ⚠️ 谨慎支持 |
...
| 主体 | 态度 | 关键表述 |
|------|------|---------|
| **Microsoft**（Brad Smith） | 欢迎 | 「在推进创新的同时保护公众安全的重要一步」 |
| **Anthropic** | 欢迎 | 「加强美国 AI 领导地位的重要步骤」 |
| **Dean Ball** | 谨慎批评 | 「这是一个小到中等的错误」——尤其担忧分类基准和 NSA 主导 |
| **Samuel Hammond** | 谨慎支持 | 「很高兴 EO 存在，但对透明度担忧」——主张由 CAISI 而非 NSA 主导 |
| **Cato Institute**（Jordan J. Ballor） | 有条件支持 | 「虽有缺陷，但方向正确」——但缺乏明确标准是隐患 |
| **Neil Chilson** | 谨慎乐观 | 「30 天比 90 天好，但漏洞仍可被用于挑选赢家输家」 |
| **Ted Cruz** | 支持行动 | 「AI 发展迅速，必须应对灾难性风险而不让渡中国」 |
